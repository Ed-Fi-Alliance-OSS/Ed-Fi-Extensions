...
IF EXISTS(SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'updatechangeversion' AND event_object_schema = 'homograph' AND event_object_table = 'contact') THEN
    EXECUTE 'DROP TRIGGER updatechangeversion ON homograph.contact';
END IF;
